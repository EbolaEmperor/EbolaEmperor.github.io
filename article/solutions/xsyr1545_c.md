...
![](http://latex.codecogs.com/svg.latex?\sum\limits_{i=1}^n\sum\limits_{j=i+1}^n\min(dis_1(i,j),dis_2(i,j)))

### 题解

在树1上进行边分治，假设分支区域内的答案全部在树1上取得。

那么，如果一个点对(u,v)的答案改在树2上取得，则应该满足![](http://latex.codecogs.com/svg.latex?g_u+g_v-g_{lca}<d_u+d_v+w)

其中![](http://latex.codecogs.com/svg.latex?g_i)表示i到树2中根节点的距离，![](http://latex.codecogs.com/svg.latex?d_i)表示i到树1上当前分治边的距离，w表示树1上当前分治边的长度

我们枚举lca和u，于是条件限制可以化为：![](http://latex.codecogs.com/svg.latex?g_v-d_v<d_u+w+g_{lca}-g_u)，其中右边都是已知的

所以可以对于当前分治区域内的所有点i，将![](http://latex.codecogs.com/svg.latex?g_i-d_i)塞进权值线段树，然后枚举树2上的lca，并启发式地枚举u，然后询问有多少个v满足条件，将满足条件的这部分答案更新即可。询问完后要将子树的线段树启发式地合并起来

这样一来，我们还需要将当前分治区域内的点在树2上建立虚树，这样才能方便地枚举lca

所以，边分治+虚树+线段树+启发式合并，慢慢码吧。~~我也就写了8.5K~~
