...
![](http://www.ebola.pro/images/xsyr1534_b_1.png)

### 题解

先考虑单个点的询问如何处理。这是经典的点分树问题，只要把点分树处理出来就可以直接询问了。点分树就是对于每个点，存一下它在点分治时每一层的根以及到每层根的距离。然后这题需要对每层的根维护一个深度计数的前缀和数组

现在考虑多个点的询问。建立虚树，对每个点询问出答案然后相加，再对于虚树上相邻两个有交集的点，将它们交集部分减去。具体地，求出它们路径上交集部分的中点，然后交集就是以这个中点为中心的一个圆，询问出这个圆包含的点数，减去即可

因为交集中点可能是点也可能是边，为了简化代码，我们对边建立虚点以保证交集中点是一个点。然后深度计数数组应该只对真实的点计数

思维难度并不大，只要会点分树和虚树的套路一切都好办，就是比较码农
