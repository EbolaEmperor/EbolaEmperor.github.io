# C. Gangsters of Treeland

这题其实是[「SDOI2017」树点涂色](https://www.lydsy.com/JudgeOnline/problem.php?id=4817)的弱化版

求平均值完全是强行添加的，就是强迫我这种fwrite党写一个浮点输优，还好我存了板子，哼😕

然后吐槽一下cky，复制我的代码，换成他的IO（还是魔改的我的），翻到我前面

说正经的。其实这个修改就是LCT的access操作。对dfs序建一棵线段树，维护区间f值之和。只要在虚实关系发生改变时，对子树区间修改即可（虚变实，子树-1；实变虚，子树+1）
