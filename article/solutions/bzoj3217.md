...
一句话题意：维护一个数列，支持插入、删除、修改、询问区间次大值与区间中其它元素的异或最大值，强制在线

一道几乎没有思维难度的题，但是很考验码力

异或最大值没什么好说的，肯定是Trie。如果只要修改，那线段树套Trie就行了。但现在有个该死的插入删除，还强制在线，于是只能用平衡树套Trie

考虑平衡树的每个节点，它上面有一棵Trie，存了它子树中的所有值。维护一个子树次大值，这个和子树最大值维护起来是一样的，就是强行要你多写几行代码。询问直接查询出区间次大，然后像线段树处理区间问题一样，把询问拆区间，然后拿这个次大值在拆出的区间对应的Trie树上跑一下异或最大值，不必将所有Trie合并，可以分开跑，跑出的答案取max就行。然后再支持一个插入删除，修改就是删除+插入喽

如果我们使用带旋转的平衡树，每次旋转，父子关系发生变化，内层Trie上势必会发生大量的元素转移，所以肯定不行，要用不带旋转的平衡树。因为我不会fhqTreap，我也不知道那玩意儿能不能做这道题，反正我是写的替罪羊树

然后，码吧。。。平衡树上的拆区间询问不像线段树一样那么好写，要特别注意边界问题
