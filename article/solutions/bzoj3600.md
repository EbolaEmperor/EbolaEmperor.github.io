...
题面很长，很考验语文水平，自己慢慢领悟一下吧，不是一两句话说得清的

 考虑到大小关系是有传递性的，所以我们可以按相对大小将所有元素排序，赋一个键值就可以O(1)比较了。由于需要支持插入，每次插入完排序肯定是行不通的，我们用平衡树来维护它们

那键值是什么？平衡树上的排名？那样比较就变成O(log n)了，因为要跑一遍平衡树的rank。有一个非常巧妙的方法，设根节点表示[0,1]区间，然后根节点本身的值设为0.5，左子节点表示[0,0.5]区间，右子节点表示[0.5,1]区间，类似地这样传下去，这样设出来的键值就满足了相对大小关系

如果采用旋转平衡树，一次旋转势必会引起子树中所有的点键值发生变化，维持平衡得不偿失，因此我们要采用非旋平衡树，可以用fhqTreap，因为我不会所以我写的替罪羊树

然后就是简单的带修改区间最值问题，直接上线段树就可以解决了

注意如果使用double，那替罪羊的平衡因子不能超过0.75，否则会由于深度过大导致键值爆精度。当然你说你用long double吧，那你的常数就很堪忧了，而且平衡因子大于0.75可能还会从替罪羊树自身的层面上更慢
