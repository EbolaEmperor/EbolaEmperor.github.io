...
![](http://www.ebola.pro/images/xsyr1526_c_1.png)
...
随机若干次，每次按题目所说的方法去赋权，然后把所有二次函数的极值拎出来排个序，看第(k+1)/2大的是否大于0.618。大于的次数除以随机总次数就近似于概率

但由于时限问题，随机次数不能过多，这就决定了我们的误差可能会比较大。又由于本题的精度要求极高，如果把随机程序交上去，每个点估计也就3分的样子。但是考虑到可能的输入数据非常有限，所以可以本地打表

本地枚举所有可能的图，然后把随机次数设为1e8跑出表

可是n=6的时候有32768种不同的图？？？比赛时间是有限的，如果你是比赛结束之后做题，那你的人生也是有限的。~~如何在有限的人生中打出无限的表？~~

仔细思考，发现很多图的本质是相同的。也即，将点重新编号，很多图都可以变成同一个图。于是我们枚举所有图，对于每个图，我们枚举置换（也就是点重新编号），如果当前图正好置换成了一个之前算过的图，那就不再重复计算

n的值为3到6时，本质不同的图数量分别为：4、11、34、156，这是一个打表可以接受的范围。如果利用起CPU的4个核，大概2小时就可以打完

再提一点关于图的存储。可以将矩阵中i < j的位置按顺序连起来，看做一个二进制数，然后就可以用一个int来表示一个图
